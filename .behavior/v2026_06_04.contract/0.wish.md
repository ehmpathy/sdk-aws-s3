...
lets build out this sdk

here's what a consumer wants
...
|-----------|-------|--------|
| `get.one` | `{ uri }` or `{ bucket, key }` | `string \| undefined` |
...
`get.one` returns `undefined` for not-found, never throws.
...
await sdkAwsS3.get.one(
  ...
  { { aws: { s3: { client } } } // ContextSdkAwsS3
);
```

### tests

integration tests against real S3 (see how simple-on-disk-cache setup test bucket):

1. `get.one` returns `undefined` for non-existent key
